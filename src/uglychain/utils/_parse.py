...
def _parse_json(response: str) -> dict[str, object]:
    try:
        ...
        if match:
            ...
            return {k: _convert_value_type(v) if isinstance(v, str) else v for k, v in parsed_dict.items()}
        else:
...
def parse_to_dict(response: str) -> dict[str, object]:
    if not response.strip():
        return {}
    if xml_not_json_regex.match(response):
        args = xml_param_regex.findall(response)
        if args:
            return {param: _convert_value_type(value) for param, value in args}
